		...
		,fn_SearchGrid:function(){
			...
			if($('#start_dt').val()==''){
				...
				$("#notice_list").jqGrid({
					...
					,rownumbers : false
					,viewrecords: true
					,colNames:["고유번호","상태","번호","제목","현재상태","등록일시","수정일시"]
					,colModel:[
					...
					,gridComplete : function(data,status) {
						var val = jQuery("#notice_list").jqGrid('getGridParam','records');
						$("#totalCnt").html(val);

						// 그리드 데이터의 ID 가져오기
						var ids = $('#notice_list').jqGrid('getDataIDs');
					//	alert(ids);
						// 그리드 데이터 가져오기
						var gridData = $("#notice_list").jqGrid('getRowData');
						// 데이터 확인후 색상 변경
			    	...
			    	,ondblClickRow: function (rowid, iRow, iCol, e) {
				    	var ret = jQuery("#notice_list").jqGrid('getRowData',rowid);
				    	//상세보기
				    	goView(ret.BD_SQ);
			    	}
...
//	alert(val);
	$('#bd_sq').val(val);
	$('#selectedShForm').attr('action', '/board/noticeView');
	$("#selectedShForm").submit();
}
